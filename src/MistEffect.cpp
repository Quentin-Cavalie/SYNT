...
MistEffect::MistEffect(unsigned int numParticles /* = 0 */)
: m_pCamera( NULL )
...
bool MistEffect::Active()
{
    return isActive;
}

void MistEffect::SetCamera( Camera* pCamera )
{
    m_pCamera = pCamera;
}

void MistEffect::SetParticleEmitter( ParticleEmitter* pEmitter )
{
    m_pParticleEmitter = pEmitter;
}

void MistEffect::SetColorInterplator( const ColorInterpolator& colors )
{
    m_ColorInterpolator = colors;
}

bool MistEffect::LoadTexture( const std::string& fileName )
{
    // if ( m_TextureID != 0 )
    //     glDeleteTextures(1, &m_TextureID );
...
void MistEffect::EmitParticle( Particle& particle )
{
    assert( m_pParticleEmitter != NULL );
    m_pParticleEmitter->EmitParticle( particle );
}

void MistEffect::EmitParticles()
{
...
void MistEffect::BuildVertexBuffer()
{
...
void MistEffect::Update(float fDeltaTime)
{
    ...
    for ( unsigned int i = 0; i < m_Particles.size(); ++i )
    {
        ...
        else
        {
            ...
            particle.m_fSize = glm::lerp<float>( 10.0f, 70.0f, lifeRatio );
        }
    }
...
void MistEffect::Resize( unsigned int numParticles )
{

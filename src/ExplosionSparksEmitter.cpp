...
ExplosionSparksEmitter::ExplosionSparksEmitter()
: MinimumRadius(12)
, MaximumRadius(15)
, MinInclination(0)
, MaxInclination(360)
...
void ExplosionSparksEmitter::setOrigin(glm::vec3 origin)
{
    this->Origin = origin;
}

void ExplosionSparksEmitter::EmitParticle( Particle& particle )
{
    ...
    particle.m_Velocity = RandExplosionSparksUnitVec() * speed;
...
void ExplosionSparksEmitter::RenderSphere( glm::vec4 color, float fRadius )
{
    float X, Y, Z, inc, azi;
...
void ExplosionSparksEmitter::DebugRender()
{

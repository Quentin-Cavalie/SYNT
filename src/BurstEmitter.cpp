...
void BurstEmitter::setOrigin(glm::vec3 origin)
{
    this->Origin = origin;
}

void BurstEmitter::EmitParticle( Particle& particle )
{
    ...
    particle.m_Velocity = RandBurstUnitVec() * speed;
...
void BurstEmitter::RenderSphere( glm::vec4 color, float fRadius )
{
    float X, Y, Z, inc, azi;

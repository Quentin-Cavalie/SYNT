...
BurstEffect::BurstEffect(unsigned int numParticles /* = 0 */)
: m_pCamera( NULL )
...
bool BurstEffect::Active()
{
    return isActive;
}

void BurstEffect::SetCamera( Camera* pCamera )
{
    m_pCamera = pCamera;
}

void BurstEffect::SetParticleEmitter( ParticleEmitter* pEmitter )
{
    m_pParticleEmitter = pEmitter;
}

void BurstEffect::SetColorInterplator( const ColorInterpolator& colors )
{
    m_ColorInterpolator = colors;
}

bool BurstEffect::LoadTexture( const std::string& fileName )
{
    if ( m_TextureID != 0 )
        glDeleteTextures(1, &m_TextureID );
...
void BurstEffect::EmitParticle( Particle& particle )
{
    assert( m_pParticleEmitter != NULL );
    m_pParticleEmitter->EmitParticle( particle );
}

void BurstEffect::EmitParticles()
{
...
void BurstEffect::BuildVertexBuffer()
{
...
void BurstEffect::Update(float fDeltaTime)
{
    float drag = 12.f;
    ...
    for ( unsigned int i = 0; i < m_Particles.size(); ++i )
    {
        ...
        else
        {
            ...
            particle.m_Velocity += ( m_Force * fDeltaTime ) / drag;
            particle.m_Position += ( particle.m_Velocity * fDeltaTime );
            particle.m_Color = m_ColorInterpolator.GetValue( lifeRatio );
            particle.m_fRotate = glm::lerp<float>( 0.0f, particle.m_ClockRot * 0.9f, lifeRatio );
            particle.m_fSize = glm::lerp<float>( 0.05f, 60.0f, lifeRatio );
        }
    }
...
void BurstEffect::Resize( unsigned int numParticles )
{

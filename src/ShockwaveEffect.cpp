...
ShockwaveEffect::ShockwaveEffect(unsigned int numParticles /* = 0 */)
: m_pCamera( NULL )
...
, isActive(true)
{
  m_Rot = RandRange(0.0f,360.0f);
  Resize(numParticles);
}

bool ShockwaveEffect::Active()
{
    return isActive;
}
...
void ShockwaveEffect::SetCamera( Camera* pCamera )
{
    m_pCamera = pCamera;
}

void ShockwaveEffect::SetParticleEmitter( ParticleEmitter* pEmitter )
{
    m_pParticleEmitter = pEmitter;
}

void ShockwaveEffect::SetColorInterplator( const ColorInterpolator& colors )
{
    m_ColorInterpolator = colors;
}

bool ShockwaveEffect::LoadTexture( const std::string& fileName )
{
    if ( m_TextureID != 0 )
        glDeleteTextures(1, &m_TextureID );
...
void ShockwaveEffect::EmitParticle( Particle& particle )
{
    assert( m_pParticleEmitter != NULL );
    m_pParticleEmitter->EmitParticle( particle );
}

void ShockwaveEffect::EmitParticles()
{
...
void ShockwaveEffect::BuildVertexBuffer()
{
    ...
    for ( unsigned int i = 0; i < m_Particles.size(); ++i )
    {
        ...
        glm::quat rotation = glm::angleAxis( particle.m_fRotate, Y );
...
void ShockwaveEffect::Update(float fDeltaTime)
{
    ...
    for ( unsigned int i = 0; i < m_Particles.size(); ++i )
    {
        ...
        //  particle.m_fRotate = glm::lerp<float>( 0.0f, 5.0f, lifeRatio );
            particle.m_fSize = glm::lerp<float>( 2.0f, 200.0f, lifeRatio );
        }
    }
...
void ShockwaveEffect::Resize( unsigned int numParticles )
{

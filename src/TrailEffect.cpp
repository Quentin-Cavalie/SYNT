...
TrailEffect::TrailEffect(unsigned int numParticles /* = 0 */)
: m_pCamera( NULL )
...
void TrailEffect::SetCamera( Camera* pCamera )
{
    m_pCamera = pCamera;
}

void TrailEffect::SetParticleEmitter( ParticleEmitter* pEmitter )
{
    m_pParticleEmitter = pEmitter;
}

void TrailEffect::SetColorInterplator( const ColorInterpolator& colors )
{
    m_ColorInterpolator = colors;
}

bool TrailEffect::LoadTexture( const std::string& fileName )
{
    if ( m_TextureID != 0 )
...
void TrailEffect::EmitParticle( Particle& particle )
{
    if (m_pParticleEmitter != NULL)
	    m_pParticleEmitter->EmitParticle( particle );
}

void TrailEffect::EmitParticles()
{
    if ( m_pParticleEmitter == NULL )
    {
        RandomizeParticles();
...
void TrailEffect::BuildVertexBuffer()
{
...
void TrailEffect::Update(float fDeltaTime)
{
    ...
    for ( unsigned int i = 0; i < m_Particles.size(); ++i )
    {
        ...
        if ( particle.m_fAge > particle.m_fLifeTime )
        {
            ...
            glm::vec4 color = m_ColorInterpolator.GetValue( lifeRatio );
           	color.a = color.a * (1 - m_pParticleEmitter->getAge() / m_pParticleEmitter->getMaxLife());
            particle.m_Color = color;
        //particle.m_fRotate = glm::lerp<float>( 0.0f, 0.0f, lifeRatio );
            particle.m_fSize = glm::lerp<float>( 3.0f, 7.0f, lifeRatio );
    }
...
void TrailEffect::Resize( unsigned int numParticles )
{

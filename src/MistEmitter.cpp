...
void MistEmitter::setOrigin(glm::vec3 origin)
{
    this->Origin = origin;
}

void MistEmitter::EmitParticle( Particle& particle )
{
    ...
    particle.m_Velocity = RandMistUnitVec() * speed;
...
void MistEmitter::RenderSphere( glm::vec4 color, float fRadius )
{
    float X, Y, Z, inc, azi;

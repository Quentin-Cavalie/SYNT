...
void SparkEffect::SetCamera( Camera* pCamera )
{
    m_pCamera = pCamera;
}

void SparkEffect::SetParticleEmitter( ParticleEmitter* pEmitter )
{
    m_pParticleEmitter = pEmitter;
}

void SparkEffect::SetColorInterplator( const ColorInterpolator& colors )
{
    m_ColorInterpolator = colors;
}

bool SparkEffect::LoadTexture( const std::string& fileName )
{
    if ( m_TextureID != 0 )
    {
        glDeleteTextures(1, &m_TextureID );
...
void SparkEffect::RandomizeParticle( Particle& particle )
{
...
void SparkEffect::RandomizeParticles()
{
...
void SparkEffect::EmitParticle( Particle& particle )
{
    assert( m_pParticleEmitter != NULL );
    m_pParticleEmitter->EmitParticle( particle );
}

void SparkEffect::EmitParticles()
{
...
void SparkEffect::BuildVertexBuffer()
{
...
void SparkEffect::Update(float fDeltaTime)
{
    ...
    for ( unsigned int i = 0; i < m_Particles.size(); ++i )
    {
       ...
       // particle.m_Color = m_ColorInterpolator.GetValue(((abs(particle.m_Position.x) * 2.3 + abs(particle.m_Position.y) * 1.2 + abs(particle.m_Position.z) * 3.3)) * lifeRatio * 0.5);
        particle.m_fRotate = glm::lerp<float>( 0.0f, 3600.0f, lifeRatio );
        particle.m_fSize = glm::lerp<float>( 3.5f, 0.0f, lifeRatio );
...
void SparkEffect::Resize( unsigned int numParticles )
{

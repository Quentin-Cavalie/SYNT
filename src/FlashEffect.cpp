...
FlashEffect::FlashEffect(unsigned int numParticles /* = 0 */)
: m_pCamera( NULL )
...
void FlashEffect::SetCamera( Camera* pCamera )
{
    m_pCamera = pCamera;
}

void FlashEffect::SetParticleEmitter( ParticleEmitter* pEmitter )
{
    m_pParticleEmitter = pEmitter;
}

void FlashEffect::SetColorInterplator( const ColorInterpolator& colors )
{
    m_ColorInterpolator = colors;
}

bool FlashEffect::LoadTexture( const std::string& fileName )
{
    if ( m_TextureID != 0 )
        glDeleteTextures(1, &m_TextureID );
...
void FlashEffect::EmitParticle( Particle& particle )
{
    assert( m_pParticleEmitter != NULL );
    m_pParticleEmitter->EmitParticle( particle );
}

void FlashEffect::EmitParticles()
{
...
void FlashEffect::BuildVertexBuffer()
{
...
void FlashEffect::Update(float fDeltaTime)
{
    ...
    for ( unsigned int i = 0; i < m_Particles.size(); ++i )
    {
        ...
        else
        {
            ...
            particle.m_fRotate = glm::lerp<float>( 0.0f, particle.m_ClockRot * 0.1f, lifeRatio );
            particle.m_fSize = glm::lerp<float>( 5.f, 65.0f, lifeRatio );
        }
    }
...
void FlashEffect::Resize( unsigned int numParticles )
{

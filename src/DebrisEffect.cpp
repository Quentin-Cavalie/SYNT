...
DebrisEffect::DebrisEffect(unsigned int numParticles /* = 0 */)
: m_pCamera( NULL )
...
void DebrisEffect::SetCamera( Camera* pCamera )
{
    m_pCamera = pCamera;
}

void DebrisEffect::SetParticleEmitter( ParticleEmitter* pEmitter )
{
    m_pParticleEmitter = pEmitter;
}

void DebrisEffect::SetColorInterplator( const ColorInterpolator& colors )
{
    m_ColorInterpolator = colors;
}

bool DebrisEffect::LoadTexture( const std::string& fileName )
{
    if ( m_TextureID != 0 )
        glDeleteTextures(1, &m_TextureID );
...
void DebrisEffect::EmitParticle( Particle& particle )
{
    assert( m_pParticleEmitter != NULL );
    m_pParticleEmitter->EmitParticle( particle );
}

void DebrisEffect::EmitParticles()
{
...
void DebrisEffect::BuildVertexBuffer()
{
...
void DebrisEffect::Update(float fDeltaTime)
{
    ...
    for ( unsigned int i = 0; i < m_Particles.size(); ++i )
    {
        ...
        else
        {
            ...
            particle.m_fRotate = glm::lerp<float>( 0.0f, particle.m_ClockRot * 4.0f, lifeRatio );
            particle.m_fSize = glm::lerp<float>( 1.0f, 4.0f, lifeRatio );
        }
    }
...
void DebrisEffect::Resize( unsigned int numParticles )
{

...
void ShockwaveEmitter::setOrigin(glm::vec3 origin)
{
    this->Origin = origin;
}

void ShockwaveEmitter::EmitParticle( Particle& particle )
{
    ...
    particle.m_Velocity = RandShockwaveUnitVec() * speed;

    particle.m_fLifeTime = lifetime;
    particle.m_fAge = 0;

    particle.m_fRotate = RandRange(-10, 10);
}

void ShockwaveEmitter::RenderSphere( glm::vec4 color, float fRadius )
{
    float X, Y, Z, inc, azi;
...
void ShockwaveEmitter::DebugRender()
{

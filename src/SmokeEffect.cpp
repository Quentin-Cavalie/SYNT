...
void SmokeEffect::SetCamera( Camera* pCamera )
{
    m_pCamera = pCamera;
}

void SmokeEffect::SetParticleEmitter( ParticleEmitter* pEmitter )
{
    m_pParticleEmitter = pEmitter;
}

void SmokeEffect::SetColorInterplator( const ColorInterpolator& colors )
{
    m_ColorInterpolator = colors;
}

bool SmokeEffect::LoadTexture( const std::string& fileName )
{
    if ( m_TextureID != 0 )
    {
        glDeleteTextures(1, &m_TextureID );
...
void SmokeEffect::RandomizeParticle( Particle& particle )
{
    particle.m_fAge = 0.0f;
    particle.m_fLifeTime = RandRange( 0.5, 0.8 );

    glm::vec3 unitVec = RandSmokeUnitVec();

    particle.m_Position = unitVec * 1.0f;
    particle.m_Velocity = unitVec * RandRange( 10, 20 );
}

void SmokeEffect::RandomizeParticles()
{
...
void SmokeEffect::EmitParticle( Particle& particle )
{
    assert( m_pParticleEmitter != NULL );
    m_pParticleEmitter->EmitParticle( particle );
}

void SmokeEffect::EmitParticles()
{
...
void SmokeEffect::BuildVertexBuffer()
{
...
void SmokeEffect::Update(float fDeltaTime)
{
    ...
    for ( unsigned int i = 0; i < m_Particles.size(); ++i )
    {
       ...
       // particle.m_Color = m_ColorInterpolator.GetValue(((abs(particle.m_Position.x) * 2.3 + abs(particle.m_Position.y) * 1.2 + abs(particle.m_Position.z) * 3.3)) * lifeRatio * 0.5);
        particle.m_fRotate = glm::lerp<float>( 0.0f, 3000.0f * particle.m_ClockRot, lifeRatio );
        particle.m_fSize = glm::lerp<float>( 2.0f, 8.5f, lifeRatio );
...
void SmokeEffect::Resize( unsigned int numParticles )
{

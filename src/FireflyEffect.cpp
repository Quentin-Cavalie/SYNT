...
FireflyEffect::FireflyEffect(unsigned int numParticles /* = 0 */)
: m_pCamera( NULL )
...
void FireflyEffect::SetCamera( Camera* pCamera )
{
    m_pCamera = pCamera;
}

void FireflyEffect::SetParticleEmitter( ParticleEmitter* pEmitter )
{
    m_pParticleEmitter = pEmitter;
}

void FireflyEffect::SetColorInterplator( const ColorInterpolator& colors )
{
    m_ColorInterpolator = colors;
}

bool FireflyEffect::LoadTexture( const std::string& fileName )
{
    // if ( m_TextureID != 0 )
...
void FireflyEffect::RandomizeParticle( Particle& particle )
{
...
void FireflyEffect::RandomizeParticles()
{
...
void FireflyEffect::EmitParticle( Particle& particle )
{
    assert( m_pParticleEmitter != NULL );
    m_pParticleEmitter->EmitParticle( particle );
}

void FireflyEffect::EmitParticles()
{
...
void FireflyEffect::Update(float fDeltaTime)
{
    ...
    for ( unsigned int i = 0; i < m_Particles.size(); ++i )
    {
        ...
        else
        {
            ...
            particle.m_fSize = glm::lerp<float>( 0.7f, 0.0f, lifeRatio );
        }
    }
...
void FireflyEffect::Resize( unsigned int numParticles )
{
